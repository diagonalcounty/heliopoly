# App Store listing draft (#232)

**Read [WHY.md](./WHY.md) first.** This file is proposed / locked lines only.

**Live today:** subtitle `Orbital Economics`; promo mixes strategy + offline iPad; description is feature-forward; no Angzarr in promo. Price **$3.99**.

---

## Locked picks (see WHY for reasons)

| Field | Text |
|-------|------|
| Subtitle | `Space Economics. Trade in ⍼.` |
| Promo | `The only App Store game where the currency is Angzarr (⍼).` |
| Opener | Helios / Mainline; **currency** is Angzarr (⍼); last rocket; no timer |
| Devices | iPhone & iPad in body (not promo) |
| Homeschool | Not in listing |

---

## Description — proposed opener

```
Heliopoly is orbital economics on the Mainline of our solar system (Helios): buy claims, burn propellant, duel rivals, and try not to go broke.

Currency is Angzarr (⍼). Every buy, rent, and duel hits the Automated Interplanetary Asset Ledger. Last rocket flying wins — no timer, no “enough Angzarr.”

You name a rocket, pick methane or hydrogen, and compete against AI pilots from Mercury to the Saturn moons. Own systems, plant fuel depots, and stay solvent long enough for the ledger to write your name.
```

Then keep scrubbed HOW AN EXPEDITION FEELS / PLAY YOUR WAY / OFFLINE blocks from live copy; disclaimer unchanged; **no** pod sentence.

## Screenshot note

At least one caption or in-frame UI shows `⍼` amounts.

## Line bank (not promo)

From Jacob — use in description body or caption; **do not** replace P1 without an explicit promo swap (see WHY §5):

> As of 2026 this is the only space game where a rogue tesla can crash into your property and destroy your fuel pods.

## Acceptance (#232)

- [x] Subtitle A locked (reason: WHY §1)
- [x] Promo P1 locked — single Angzarr pitch (reason: WHY §1)
- [x] Opener Helios + currency (reason: WHY §1)
- [x] No public homeschool claim (reason: WHY §2)
- [ ] Screenshot shows `⍼`
- [x] Price $3.99
- [ ] ASC patch after Jacob says go (Steve)
