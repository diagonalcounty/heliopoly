...

{
  ...
  assert(wide.eggRx >= 38 && wide.eggRx <= 41 && wide.eggRy >= 36 && wide.eggRy <= 39, "wide egg diameter ~75–82% of viewBox");
  assert(EGG_BODY_DEFAULTS.armLength >= 9 && EGG_BODY_DEFAULTS.armLength <= 12, "arms short relative to shell");
  assert(EGG_BODY_DEFAULTS.endCapAspect >= 0.25 && EGG_BODY_DEFAULTS.endCapAspect <= 0.32, "end-caps are flat discs");
  ...
  assert(tall.eggRx >= 36 && tall.eggRx <= 38 && tall.eggRy >= 46 && tall.eggRy <= 48, "straight tall bias scaled with larger body");
